...
const getAllRuns = async (req, res) => {
  ...
  //Get all runs (not limited to the limit)
  const totalRuns = await Run.countDocuments(queryObject);
  const numOfPages = Math.ceil(totalRuns / limit);

  res.status(StatusCodes.OK).json({ totalRuns, numOfPages, runs });
};
